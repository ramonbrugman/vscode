...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private doOpen(
		...
		if (allWorkspacesToOpen.length > 0) {
			...
			if (windowsOnWorkspace.length > 0) {
				...
				const filesToOpenForWindow = (filesToOpen?.remoteAuthority === windowOnWorkspace.remoteAuthority) ? filesToOpen : undefined;

				// Do open files
				usedWindows.push(this.doOpenFilesInExistingWindow(openConfig, windowOnWorkspace, filesToOpenForWindow));

				// Reset these because we handled them
				if (filesToOpenForWindow) {
					filesToOpen = undefined;
				}
			...
			allWorkspacesToOpen.forEach(workspaceToOpen => {
				...
				const filesToOpenForWindow = (filesToOpen?.remoteAuthority === remoteAuthority) ? filesToOpen : undefined;

				// Do open folder
				usedWindows.push(this.doOpenFolderOrWorkspace(openConfig, workspaceToOpen, openFolderInNewWindow, filesToOpenForWindow));
		...
		if (allFoldersToOpen.length > 0) {
			...
			if (windowsOnFolderPath.length > 0) {
				...
				const filesToOpenForWindow = filesToOpen?.remoteAuthority === windowOnFolderPath.remoteAuthority ? filesToOpen : undefined;

				// Do open files
				usedWindows.push(this.doOpenFilesInExistingWindow(openConfig, windowOnFolderPath, filesToOpenForWindow));
			...
			allFoldersToOpen.forEach(folderToOpen => {
				...
				// Do open folder
				usedWindows.push(this.doOpenFolderOrWorkspace(openConfig, folderToOpen, openFolderInNewWindow, filesToOpenForWindow));
		...
		if (allEmptyToRestore.length > 0) {
			allEmptyToRestore.forEach(emptyWindowBackupInfo => {
				...
				usedWindows.push(this.openInBrowserWindow({
					...
					cli: openConfig.cli,
					initialStartup: openConfig.initialStartup,
					filesToOpen: filesToOpenForWindow,
					remoteAuthority,
					forceNewWindow: true,
					forceNewTabbedWindow: openConfig.forceNewTabbedWindow,
	...
	private getPathsToOpen(openConfig: IOpenConfiguration): IPathToOpen[] {
		...
		if (openConfig.initialStartup && !restoredWindows && this.configurationService.getValue<IWindowSettings>('window').startup === 'restore') {
			for (const windowToRestore of this.doGetWindowsFromLastSession()) {
				if (windowToRestore.folderUri || windowToRestore.workspace || windowToRestore.backupPath) {
					windowsToOpen.unshift(windowToRestore); // add to the beginning to keep the last ones to get focus
				}
			}
		}
