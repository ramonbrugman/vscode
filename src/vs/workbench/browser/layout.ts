...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	private resolveEditorsToOpen(fileService: IFileService): Promise<IResourceEditorInputType[]> | IResourceEditorInputType[] {
		...
		// or when `window.startup` setting is explicitly set to `restore`
		const forceRestoreEditors = this.configurationService.getValue<'default' | 'restore'>('window.startup') === 'restore';
