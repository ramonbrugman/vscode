...
class DropOverlay extends Themable {
	...
	private registerListeners(container: HTMLElement): void {
		this._register(new DragAndDropObserver(container, {
			...
			onDragOver: e => {
				...
				// Position overlay
				let enableSplitting = !!this.editorGroupService.partOptions.dragAndDropToSplit;
				if (this.isToggleSplitOperation(e)) {
					enableSplitting = !enableSplitting;
				}
				this.positionOverlay(e.offsetX, e.offsetY, isDraggingGroup, enableSplitting);
	...
	private isToggleSplitOperation(e: DragEvent): boolean {
		return (e.altKey && !isMacintosh) || (e.metaKey && isMacintosh);
	}
